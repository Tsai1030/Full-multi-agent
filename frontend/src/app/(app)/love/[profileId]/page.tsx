...
export default function LoveChatPage() {
  ...
  const analysisResult = typeof window !== "undefined"
    ? sessionStorage.getItem(`love-result-${profileId}`)
    : null;
  const chartText = typeof window !== "undefined"
    ? sessionStorage.getItem(`love-chart-${profileId}`) || ""
    : "";
  ...
  if (error || !analysisResult) {
    return (
      <div className="h-full flex flex-col items-center justify-center gap-4 text-center px-4">
        ...
        <Link href="/love" className="text-gold-400 text-sm hover:text-gold-300">
          返回感情姻緣算命
        </Link>
      </div>
    );
    ...
    <div className="h-full flex flex-col">
        ...
        <div className="max-w-3xl mx-auto flex items-center justify-between gap-3">
          <div className="min-w-0">
            <h1 className="font-serif text-lg sm:text-xl font-bold text-parchment leading-tight">
              <span className="text-rose-400">❤</span> 感情姻緣算命
            </h1>
            {profile && (
      ...
      <div className="flex-1 min-h-0 flex flex-col">
        <details className="shrink-0 border-b border-gold-800/20" open>
          <summary className="px-4 md:px-6 py-3 cursor-pointer text-sm text-rose-400 hover:text-rose-300 transition-colors">
            ❤ 查看分析報告
          </summary>
